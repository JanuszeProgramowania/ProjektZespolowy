...
public final class TokenHandler {
    ...
    public UserDetailsDTO parseUserFromToken(String token) {
        ...
        if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
            try {
                ...
                if (validHash) {
                    final UserDetailsDTO user = fromJSON(userBytes);
    ...
    private UserDetailsDTO fromJSON(final byte[] userBytes) {
        try {
            return new ObjectMapper().readValue(new ByteArrayInputStream(userBytes), UserDetailsDTO.class);
        } catch (IOException e) {
